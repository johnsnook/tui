...
use Tui;
use tui\helpers\VarDumper;
use tui\web\HttpException;

/**
 * ErrorHandler handles uncaught PHP errors and exceptions.
 *
 * ErrorHandler is configured as an application component in [[\tui\base\Application]] by default.
 * You can access that instance via `Tui::$app->errorHandler`.
 *
...
abstract class ErrorHandler extends Component
{
    ...
    protected function handleFallbackExceptionMessage($exception, $previousException)
    {
        ...
        if (TUI_DEBUG) {
            if (PHP_SAPI === 'cli') {
                ...
                echo '<pre>' . htmlspecialchars($msg, ENT_QUOTES, Tui::$app->charset) . '</pre>';
            }
        } else {
    ...
    public function handleError($code, $message, $file, $line)
    {
        if (error_reporting() & $code) {
            ...
            if (!class_exists('tui\\base\\ErrorException', false)) {
    ...
    public function logException($exception)
    {
        ...
        if ($exception instanceof HttpException) {
            $category = 'tui\\web\\HttpException:' . $exception->statusCode;
        } elseif ($exception instanceof \ErrorException) {
            $category .= ':' . $exception->getSeverity();
        }
        Tui::error($exception, $category);
    }
