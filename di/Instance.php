 ...
 *
 * ```php
 * $container = new \tui\di\Container;
 * $container->set('cache', [
 *     'class' => 'tui\caching\DbCache',
 *     'db' => Instance::of('db')
 * ]);
 * $container->set('db', [
 ...
 *     {
 *         parent::init();
 *         $this->db = Instance::ensure($this->db, 'tui\db\Connection');
 *     }
 * }
 * ```
 *
...
class Instance
{
     ...
     *
     * If you do not specify a container, the method will first try `Tui::$app` followed by `Tui::$container`.
     *
     * For example,
     *
     * ```php
     ...
     * If null, the method will first try `Tui::$app` then `Tui::$container`.
    ...
    public function get($container = null)
    {
        ...
        if (Tui::$app && Tui::$app->has($this->id)) {
            return Tui::$app->get($this->id);
        }

        return Tui::$container->get($this->id);
    }
